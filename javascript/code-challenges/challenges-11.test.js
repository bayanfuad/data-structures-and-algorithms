...
function transformToLis(obj){
  // Solution code here...
  const newArr = [];
  for (let [val1 , val2] of Object.entries(obj)){
    newArr.push(`<li>${val1}: ${val2}</li>`);
  }
  return newArr;
};
...
const count = (target, input) => {
  ...
  input.map((element1) =>{
    element1.map((element2) =>{
      if (element2 === target){newArr.push('x');}
    });
  });
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length ; i++){
    let newArr2 = input[i];
    newArr1.push(newArr2.filter((num) => ((num % 5 === 0) && typeof (num) ==='number')).map(value => Math.pow(2,value)));
  }
  return newArr1;
};
